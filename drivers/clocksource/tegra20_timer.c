...
static int __init tegra20_init_timer(struct device_node *np)
{
	...
	ret = clocksource_mmio_init(timer->base + TIMERUS_CNTR_1US,
...
static int __maybe_unused tegra_timer_suspend(struct device *dev)
{
...
static int __maybe_unused tegra_timer_resume(struct device *dev)
{
	struct tegra_timer *timer = dev_get_drvdata(dev);
