...
#define  SOR_DP_PADCTL0_PAD_CAL_PD BIT(23)
#define  SOR_DP_PADCTL0_TX_PU_ENABLE BIT(22)
#define  SOR_DP_PADCTL0_TX_PU_VALUE_MASK (0xff << 8)
#define  SOR_DP_PADCTL0_TX_PU_VALUE(x) (((x) & 0xff) << 8)
#define  SOR_DP_PADCTL0_PD_TXD_3 BIT(3)
#define  SOR_DP_PADCTL0_PD_TXD_0 BIT(2)
#define  SOR_DP_PADCTL0_PD_TXD_1 BIT(1)
#define  SOR_DP_PADCTL0_PD_TXD_2 BIT(0)
...
static void tegra_sor_mode_set(struct tegra_sor *sor,
			       ...
			       struct tegra_sor_state *state)
{
	struct tegra_crtc *crtc = to_tegra_crtc(sor->encoder.crtc);
	...
	writel(value, sor->regs + SOR_HEAD_STATE1(crtc->pipe));
	...
	writel(value, sor->regs + SOR_HEAD_STATE2(crtc->pipe));
	...
	writel(value, sor->regs + SOR_HEAD_STATE3(crtc->pipe));
	...
	writel(value, sor->regs + SOR_HEAD_STATE4(crtc->pipe));

	/* XXX interlacing support */
	writel(1, sor->regs + SOR_HEAD_STATE5(crtc->pipe));
#endif
...
static int tegra_sor_wakeup(struct tegra_sor *sor)
{
	...
	while (time_before(jiffies, timeout)) {
		...
		if (value == SOR_TEST_ACT_HEAD_OPMODE_AWAKE)
			return 0;
...
{
	struct tegra_crtc *crtc = to_tegra_crtc(encoder->crtc);
	const struct tegra_sor_hdmi_settings *settings;
	struct tegra_sor *sor = enc_to_sor(encoder);
	struct drm_display_mode *mode;
	struct tegra_sor_state *state;
	...
	value |= SOR_DP_PADCTL0_PD_TXD_3 | SOR_DP_PADCTL0_PD_TXD_0 |
		 SOR_DP_PADCTL0_PD_TXD_1 | SOR_DP_PADCTL0_PD_TXD_2;
	...
	*/
	...
	value = readl(sor->regs + SOR_DP_PADCTL(0));
	value &= ~SOR_DP_PADCTL0_PAD_CAL_PD;
	writel(value, sor->regs + SOR_DP_PADCTL(0));
	...
	value = readl(sor->regs + SOR_DP_PADCTL(0));
	value &= ~SOR_DP_PADCTL0_TX_PU_VALUE_MASK;
	value |= SOR_DP_PADCTL0_TX_PU_ENABLE;
	value |= SOR_DP_PADCTL0_TX_PU_VALUE(settings->tx_pu_value);
	writel(value, sor->regs + SOR_DP_PADCTL(0));
	...
	value = readl(crtc->regs + DC_DISP_DISP_COLOR_CONTROL);
	...
	writel(value, crtc->regs + DC_DISP_DISP_COLOR_CONTROL);

	/* set display head owner */
	value = readl(sor->regs + SOR_STATE1);
	value &= ~SOR_STATE1_ASY_OWNER_MASK;
	value |= SOR_STATE1_ASY_OWNER(1 + crtc->pipe);
	writel(value, sor->regs + SOR_STATE1);
	...
	value = readl(sor->regs + SOR_HEAD_STATE0(crtc->pipe));
	value &= ~SOR_HEAD_STATE0_RANGECOMPRESS;
	value &= ~SOR_HEAD_STATE0_DYNRANGE_MASK;
	writel(value, sor->regs + SOR_HEAD_STATE0(crtc->pipe));
	...
	value = readl(crtc->regs + DC_DISP_DISP_WIN_OPTIONS);
	value |= SOR1_ENABLE | SOR1_TIMING_CYA;
	writel(value, crtc->regs + DC_DISP_DISP_WIN_OPTIONS);

	tegra_crtc_commit(crtc);
				       ...
				       struct drm_connector_state *conn_state)
{
	...
	err = tegra_crtc_state_setup(conn_state->crtc, crtc_state, info->bpc,
				     sor->clk_parent, pclk);
	if (err < 0) {
...
static int tegra186_sor_bind(struct device *dev, struct device *master,
			     void *data)
{
	...
	for (i = 0; ; i++) {
		input = of_parse_phandle(dev->of_node, "inputs", i);
		if (!input)
			break;
...
static void tegra186_sor_unbind(struct device *dev, struct device *master,
				void *data)
{
	dev_dbg(dev, "> %s(dev=%p, master=%p, data=%p)\n", __func__, dev,
		master, data);
...
static const struct component_ops tegra186_sor_ops = {
	.bind = tegra186_sor_bind,
	.unbind = tegra186_sor_unbind,
};
...
static int tegra186_sor_probe(struct platform_device *pdev)
{
	...
	err = component_add(&pdev->dev, &tegra186_sor_ops);
	if (err < 0)
...
static int tegra186_sor_remove(struct platform_device *pdev)
{
	...
	component_del(&pdev->dev, &tegra186_sor_ops);
	free_irq(sor->hpd_irq, sor);
...
static int tegra186_sor_suspend(struct device *dev)
{
	int err = 0;
	dev_dbg(dev, "> %s(dev=%p)\n", __func__, dev);
	dev_dbg(dev, "< %s() = %d\n", __func__, err);
...
static int tegra186_sor_resume(struct device *dev)
{
	int err = 0;
	dev_dbg(dev, "> %s(dev=%p)\n", __func__, dev);
	dev_dbg(dev, "< %s() = %d\n", __func__, err);
...
static const struct dev_pm_ops tegra186_sor_pm_ops = {
	SET_RUNTIME_PM_OPS(tegra186_sor_suspend, tegra186_sor_resume, NULL)
};
...
struct platform_driver tegra186_sor_driver = {
	.driver = {
		...
		.pm = &tegra186_sor_pm_ops,
	},
	.probe = tegra186_sor_probe,
	.remove = tegra186_sor_remove,
};
