...
static void tegra_sor_edp_disable(struct drm_encoder *encoder)
{
	...
	err = tegra_io_pad_power_disable(TEGRA_IO_PAD_LVDS);
	if (err < 0)
		dev_err(sor->dev, "failed to disable power to I/O pad: %d\n",
			err);
...
static void tegra_sor_edp_enable(struct drm_encoder *encoder)
{
	...
	/* step 2 */
	err = tegra_io_pad_power_enable(TEGRA_IO_PAD_LVDS);
	if (err < 0)
		dev_err(sor->dev, "failed to enable power to I/O pad: %d\n",
			err);
...
static void tegra_sor_hdmi_disable(struct drm_encoder *encoder)
{
	...
	err = tegra_io_pad_power_disable(TEGRA_IO_PAD_HDMI);
	if (err < 0)
		dev_err(sor->dev, "failed to disable power to HDMI rail: %d\n",
			err);
...
static void tegra_sor_hdmi_enable(struct drm_encoder *encoder)
{
	...
	err = tegra_io_pad_power_enable(TEGRA_IO_PAD_HDMI);
	if (err < 0)
		dev_err(sor->dev, "failed to enable power to HDMI rail: %d\n",
			err);
