...
{
	nv_show_cursor(nv_crtc->base.dev, nv_crtc->pipe, true);
}

static void
nv04_cursor_hide(struct nouveau_crtc *nv_crtc, bool update)
{
	nv_show_cursor(nv_crtc->base.dev, nv_crtc->pipe, false);
}
...
nv04_cursor_set_pos(struct nouveau_crtc *nv_crtc, int x, int y)
{
	...
	NVWriteRAMDAC(nv_crtc->base.dev, nv_crtc->pipe,
...
nv04_cursor_set_offset(struct nouveau_crtc *nv_crtc, uint32_t offset)
{
	...
	if (drm->device.info.family == NV_DEVICE_INFO_V0_CURIE)
		nv_fix_nv40_hw_cursor(dev, nv_crtc->pipe);
}
