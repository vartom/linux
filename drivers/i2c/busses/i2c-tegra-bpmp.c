					...
					unsigned int num)
{
	...
	dev_dbg(i2c->dev, "> %s(i2c=%p, request=%p, msgs=%p, num=%u)\n",
		__func__, i2c, request, msgs, num);
				   ...
				   struct mrq_i2c_response *response)
{
	...
	dev_dbg(i2c->dev, "> %s(i2c=%p, request=%p, response=%p)\n", __func__,
		i2c, request, response);
	dev_dbg(i2c->dev, "  bus: %u\n", i2c->bus);
	...
	dev_dbg(i2c->dev, "writing %u bytes, %*ph\n", request->xfer.data_size,
		request->xfer.data_size, request->xfer.data_buf);
	...
	dev_dbg(i2c->dev, "read %u bytes: %*ph\n", response->xfer.data_size,
		response->xfer.data_size ?: 4, response->xfer.data_buf);
			       ...
			       struct i2c_msg *msgs, int num)
{
	...
	if (1) {
		...
		for (i = 0; i < num; i++) {
			dev_dbg(i2c->dev, "  %u: %02x: %s, %u bytes\n", i,
				msgs[i].addr,
...
MODULE_DESCRIPTION("Tegra BPMP I2C Bus contoller virtual driver");
